...
function specCheck(specbtn,specName,specList,Error,specAll){
	$(specbtn).click(function(){
	    ...
	    var text2="请输入规格名称";
		var text3="请输入内容";
		var text4="<div class='spec5' class='clearfix'><span class='specn'><span class='cright'>"+text+"</span><input type='text' class='specName5' value='请输入内容' onfocus=\"javascript:if(this.value=='请输入内容')this.value='';\"/><input type='button' id='specbtn5' value='新增'><span class='Error5' style='font-size: 14px;'></span></span><ul class='c4Ul' id='specList5'></ul>";
	    //判断值是否为空或为输入框的提示内容
	    if(text == ""||text == text2||text == text3){
	    	//是，则显示提示内容
	...
	//点击删除
    $(document).on("click",".c4Ul>li>a", function() {
        $(this).parent().remove();
    });
}
//例子
$(function(){
	$(document).on("click","#specbtn5", function() {
    	var texta=$(".specName5").val();
    	var text1a="<li>"+texta+"<a href='javascript:void();'><img src='img/delete2.png' alt='删除'></a></li>";
    	var text2a="请输入规格名称";
		var text3a="请输入内容";
        if(texta == ""||texta == text2a||texta == text3a){
	    	//是，则显示提示内容
	    	$(".Error5").html("<font color='red'>提示：请输入内容!</font>");
	    	//否，将文本追加在列表
	    }else{
	    	$("#specList5").append(text1a);
	    }
	    console.log(2);
    });
});
